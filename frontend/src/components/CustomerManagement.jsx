...
const SearchableDropdown = ({
  ...
  return (
    <div ref={dropdownRef} className={`relative w-full ${className}`}>
      <p className="text-[10px] font-bold text-[#768390] mb-1.5 uppercase tracking-wider">
        {label}
      </p>
      <button
        type="button"
        ...
        <div className="absolute top-full left-0 right-0 mt-2 bg-bg-card border border-border-main rounded-xl shadow-2xl z-[100] p-2 animate-in fade-in slide-in-from-top-2">
          ...
          <div className="max-h-40 overflow-y-auto no-scrollbar">
            {filteredOptions.length > 0 ? (
              ...
              <p className="text-center py-2 text-[9px] font-bold text-text-dim uppercase">
                No Match
              </p>
            )}
          </div>
...
const CustomerManagement = () => {
  ...
  const [persistedColConfig, setLocalColConfig] = usePersistentState(
    "customer_col_config",
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !colConfigRef.current.contains(event.target)
      ) {
  ...
  const filtered = (customers || []).filter((c) => {
    ...
    const customId = (c.customId || "").toString().toLowerCase();
    const email = (c.email || "").toString().toLowerCase();
    const gst = (c.gst || "").toString().toLowerCase();
    const pan = (c.pan || "").toString().toLowerCase();
    const city = (c.city || "").toString().toLowerCase();
    const address = (c.address || "").toString().toLowerCase();
  ...
  const handleDeleteCustomer = async (id, e) => {
    ...
    if (
      await confirmAction(
        "Purge Client Record?",
        "This will permanently remove the client and all associated access protocols.",
      )
    ) {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h2 className="text-3xl font-black text-text-main tracking-tighter uppercase italic flex items-center gap-3">
              <UserCheck className="text-[#f0883e]" size={32} /> Client
              Management
            </h2>
            <p className="text-[10px] font-bold text-text-dim uppercase tracking-[0.3em] mt-1 flex items-center gap-2">
              <ShieldCheck size={12} className="text-[#3fb950]" /> Strategic
              Onboarding & Compliance Terminal
            </p>
          </div>
              ...
              <button
                ...
                className={`p-3 rounded-xl border transition-all flex items-center gap-2 text-[10px] font-black uppercase tracking-widest ${
                  showColConfig
                    ...
                    : "bg-bg-deep border border-border-main text-text-dim hover:text-text-main"
                }`}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2 p-2 rounded-lg text-[8px] font-black uppercase transition-all ${
                          localColConfig[col.id]
                            ? "bg-[#f0883e]/10 text-[#f0883e]"
                            : "bg-bg-deep text-text-dim hover:text-slate-400"
                        }`}
                      >
                        {localColConfig[col.id] ? (
                          <Check size={10} />
                        ) : (
                          <div className="w-[10px] h-[10px] border border-slate-700 rounded-sm" />
                        )}
                        {col.label}
                      </button>
                    ))}
            ...
            {["EMI", "Rental", "FMC"].map((type) => (
              <button
                ...
                className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${
                  filterType === type
                    ? "bg-[#f0883e] text-black shadow-lg shadow-orange-500/10"
          ...
          <div className="flex flex-1 max-w-2xl gap-3">
            ...
            <div ref={filtersRef} className="relative">
              <button
                ...
                className={`px-4 py-3 rounded-xl border transition-all flex items-center gap-2 text-[10px] font-black uppercase tracking-widest ${
                  showAdvancedFilters ||
                  filters.status !== "All" ||
                  filters.city !== "All"
                    ? "bg-[#f0883e]/10 border-[#f0883e]/50 text-[#f0883e]"
                ...
                <div className="absolute top-full right-0 mt-3 w-72 bg-bg-card border border-border-main rounded-2xl shadow-2xl z-[60] p-6 animate-in fade-in zoom-in-95 duration-200">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button
                      ...
                      className="text-[9px] font-bold text-[#f0883e] hover:underline uppercase flex items-center gap-1"
                    >
                  ...
                  <div className="space-y-6">
                    <div className="space-y-3">
                      <p className="text-[8px] font-bold text-text-dim uppercase tracking-[0.2em]">
                        Compliance Status
                      </p>
                      ...
                      <div className="max-h-32 overflow-y-auto no-scrollbar space-y-1">
                        {cities.map((c) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 rounded-lg text-[9px] font-bold uppercase transition-all flex items-center justify-between ${
                              filters.city === c
                                ? "bg-[#f0883e]/10 text-[#f0883e]"
          ...
          <table className="w-full text-left border-collapse min-w-max relative">
            <thead className="sticky top-0 z-[40]">
              <tr className="bg-bg-card border-b border-border-main shadow-sm">
                {localColConfig.name && (
                  <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-text-dim bg-bg-card">
                    Client Name
                  </th>
                )}
                ...
                {localColConfig.mobile && (
                  <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-text-dim bg-bg-card">
                    Contact
                  </th>
                )}
                {localColConfig.email && (
                  <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-text-dim bg-bg-card">
                    Email
                  </th>
                )}
                ...
                {localColConfig.address && (
                  <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-text-dim bg-bg-card">
                    Address
                  </th>
                )}
                {localColConfig.overdue && (
                  <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-red-500 bg-bg-card">
                    Overdue
                  </th>
                )}
                {localColConfig.control && (
                  <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-text-dim text-right sticky right-0 bg-bg-card z-[41]">
                    Ops
                  </th>
                )}
              </tr>
            ...
            <tbody className="divide-y divide-border-main/50">
              {paginatedData.length === 0 ? (
                <tr>
                  <td
                    colSpan={
                      Object.values(localColConfig).filter(Boolean).length
                    }
                    className="px-6 py-20 text-center"
                  >
                    <p className="text-[10px] font-bold text-text-dim uppercase tracking-[0.3em]">
                      No {filterType} clients found matching your search
                    </p>
                  </td>
                </tr>
              ) : (
                    ...
                    {localColConfig.name && (
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-black text-text-main text-[11px] tracking-tight group-hover:text-[#f0883e] transition-colors whitespace-nowrap">
                            {c.name || "Unknown"}
                          </span>
                        </div>
                      </td>
...
const CustomerFormModal = ({ isOpen, onClose, customer }) => {
  ...
  const checkDuplicate = (field, value) => {
    if (!value) return;
    const exists = state.data.customers.some(
      (c) => c._id !== customer?._id && c[field] === value,
    );
    if (exists) {
  ...
  const verifyBank = async () => {
    if (!formData.ifsc)
      return showNotification("Please enter IFSC code", "info");
    setIsVerifyingBank(true);
    setBankDetails(null);
    try {
      const res = await fetch(
        `https://ifsc.razorpay.com/${formData.ifsc.toUpperCase()}`,
      );
      if (res.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      showNotification(
        "Protocol Conflict: Resolve duplicates before execution",
        "error",
      );
    ...
    try {
      ...
      showNotification(
        customer ? "Profile Updated" : "Client Onboarded Successfully",
        "success",
      );
      onClose();
    } catch (err) {
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4 px-1 pb-2">
        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="group">
            <p className="text-[9px] font-bold text-text-dim mb-1 uppercase tracking-wider">
              Legal Entity Name
            </p>
            <input
              type="text"
          ...
          <div className="group">
            <div className="flex items-center justify-between mb-1">
              ...
              {numbering?.customer?.mode === "Auto" && !customer && (
                <span className="text-[7px] font-mono text-[#f0883e]">
                  AUTO
                </span>
              )}
            </div>
            ...
            />
            {validationErrors.customId && (
              <p className="text-[7px] text-red-500 font-bold uppercase mt-1 animate-pulse">
                {validationErrors.customId}
              </p>
            )}
          </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="group">
            <p className="text-[9px] font-bold text-text-dim mb-1 uppercase tracking-wider text-[#58a6ff]">
              Bank Account
            </p>
            <input
              type="text"
        ...
        <div className="grid grid-cols-4 gap-4 items-center">
          <div className="col-span-1 group">
            <p className="text-[9px] font-bold text-text-dim mb-1 uppercase tracking-widest">
              Tax ID (GSTIN)
            </p>
            <input
              type="text"
        ...
        {bankDetails && (
          <div className="bg-[#58a6ff]/5 border border-[#58a6ff]/20 rounded-xl px-4 py-2 flex items-center justify-between animate-in fade-in slide-in-from-top-1">
            ...
            <p className="text-[8px] font-mono text-[#58a6ff] font-black">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-white/5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[8px] font-black text-[#444c56] uppercase tracking-widest">
          ...
          <div className="flex items-center gap-4">
            <button
              type="button"
              onClick={onClose}
              className="px-6 py-2 text-[10px] font-black text-[#768390] uppercase tracking-widest hover:text-white transition-all"
            >
              Dismiss
            </button>
            <button
              type="submit"
              className="px-10 py-3 bg-[#f0883e] text-black text-[10px] font-black rounded-xl uppercase tracking-widest shadow-lg shadow-orange-500/20 hover:scale-105 active:scale-95 transition-all"
            >
              Execute Onboarding
            </button>
          </div>
