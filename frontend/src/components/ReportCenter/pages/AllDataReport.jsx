...
const AllDataReport = forwardRef((props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    exportCSV: () => {
      ...
      const service = serviceRef.current?.exportCSV() || { headers: [], data: [] };
      const payment = paymentRef.current?.exportCSV() || { headers: [], data: [] };

      const sections = [
        { title: "EMI LOAN & PAYMENT REPORT", data: emi, color: 'FF3B82F6' },
        { title: "FMC CONTRACT & INVOICE REPORT", data: fmc, color: 'FFA855F7' },
      ...
      const maxRows = Math.max(0, ...sections.map(sec => sec.data.data.length));
    ...
    <div className="flex flex-col w-full h-full animate-fade-in pb-12 p-2">
      <div className="flex flex-col bg-bg-card rounded-xl shadow-md border border-border-main overflow-hidden">
        ...
        <div className="flex flex-row overflow-x-auto border-b border-border-main bg-bg-main custom-scrollbar">
          ...
          >
            <div className={`w-2 h-2 rounded-full mr-2 ${activeTab === "emi" ? "bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.8)]" : "bg-transparent"}`}></div>
            EMI Loan & Payment
          </button>
          ...
          >
            <div className={`w-2 h-2 rounded-full mr-2 ${activeTab === "fmc" ? "bg-purple-500 shadow-[0_0_8px_rgba(168,85,247,0.8)]" : "bg-transparent"}`}></div>
          ...
          >
            <div className={`w-2 h-2 rounded-full mr-2 ${activeTab === "service" ? "bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.8)]" : "bg-transparent"}`}></div>
          ...
          >
            <div className={`w-2 h-2 rounded-full mr-2 ${activeTab === "payment" ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.8)]" : "bg-transparent"}`}></div>
        ...
        <div className="h-[700px] relative bg-bg-main w-full">
          <div className={`w-full h-full ${activeTab === "emi" ? "block" : "hidden"}`}>
            <EMILoanPaymentReport ref={emiRef} {...props} />
          </div>
          <div className={`w-full h-full ${activeTab === "fmc" ? "block" : "hidden"}`}>
            <FMCInvoiceReport ref={fmcRef} {...props} />
          </div>
          <div className={`w-full h-full ${activeTab === "service" ? "block" : "hidden"}`}>
            <ServiceDeskReport ref={serviceRef} {...props} />
          </div>
          <div className={`w-full h-full ${activeTab === "payment" ? "block" : "hidden"}`}>
