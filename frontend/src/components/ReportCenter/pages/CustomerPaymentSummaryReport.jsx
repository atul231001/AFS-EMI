...
const CustomerPaymentSummaryReport = forwardRef(
  (
    {
      customers = [],
      loans = [],
    ...
    useEffect(() => {
      ...
      const handleWheel = (e) => {
        // If scrolling vertically (deltaY) and not holding shift
        if (e.deltaY !== 0 && Math.abs(e.deltaX) < Math.abs(e.deltaY)) {
          // If we want to force horizontal scroll, prevent default vertical scroll
          e.preventDefault();
    ...
    const data = useMemo(() => {
      ...
      payments.forEach((p) => {
        ...
        const customer =
          customers.find(
            (c) => c._id === (p.customerId?._id || p.customerId),
          ) || {};
        const loan =
          loans.find((l) => l._id === (p.loanId?._id || p.loanId)) || {};
    ...
    const filteredData = data.filter((e) => {
      // ── local search & type dropdown ─────────────────────────────────────────
      const matchesSearch =
        e.customerName.toLowerCase().includes(search.toLowerCase()) ||
        e.referenceId.toLowerCase().includes(search.toLowerCase());
      const matchesType =
        filters.paymentType === "All Types" ||
        e.paymentType === filters.paymentType;

      // ── sidebar global filters (emi section) ─────────────────────────────────
    ...
    const totalEMIPayments = filteredData
      .filter((e) => e.paymentType === "EMI")
      .reduce((sum, e) => sum + e.amountPaid, 0);
    const totalFMCPayments = filteredData
      .filter((e) => e.paymentType === "FMC Invoice")
      .reduce((sum, e) => sum + e.amountPaid, 0);
    ...
    useImperativeHandle(ref, () => ({
      exportCSV: () => {
        ...
        const dataToExport = filteredData.map((e) => [
          ...
          e.amountPaid,
          e.paymentDate,
          e.methodTransactionId,
          e.outstandingBalance,
        ]);
        return {
          headers,
          data: dataToExport,
          fileName: "Customer_Payment_Summary",
        };
      },
      ...
      <div className="flex flex-col gap-6 min-h-0 h-full flex-1 w-full animate-fade-in">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 shrink-0">
          ...
          <StatCard
            icon={CreditCard}
            label="EMI COLLECTED"
            value={formatINR(totalEMIPayments)}
            accent="text-blue-500"
          />
          <StatCard
            icon={FileText}
            label="FMC COLLECTED"
            value={formatINR(totalFMCPayments)}
          ...
          <div className="px-6 py-4 border-b border-border-main bg-bg-active/50 flex flex-col gap-4 shrink-0">
            <div className="flex items-center justify-between">
              <h3 className="text-[10px] font-black text-text-dim uppercase tracking-[0.2em] flex items-center gap-2">
                <User size={14} className="text-primary" /> CUSTOMER PAYMENT
                SUMMARY
              </h3>
                ...
                {filteredData.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest border ${row.paymentType === "EMI" ? "bg-blue-500/10 text-blue-500 border-blue-500/20" : "bg-purple-500/10 text-purple-500 border-purple-500/20"}`}
                      >
                        {row.paymentType}
                      </span>
                    </td>
