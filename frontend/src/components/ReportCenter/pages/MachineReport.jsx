...
const MachineReport = forwardRef(
  (
    {
      machines = [],
      loans = [],
    ...
    const realMachines = useMemo(() => {
      return machines.map((m) => {
        const machineLoans = loans.filter(
          (l) => l.machineId === m._id || l.machineName === m.name,
        );
        const isRented = machineLoans.some(
        ...
        return {
          id:
            m.serialNumber ||
            (m._id ? `MCH-${m._id.toString().slice(-6).toUpperCase()}` : "UNK"),
          name: m.name || "Unknown",
          category: m.category || "Uncategorized",
          brand: m.specs?.engineModel
            ? m.specs.engineModel.split(" ")[0]
            : "General",
          model: m.model || "N/A",
    ...
    const filteredMachines = realMachines.filter((m) => {
      ...
      const matchesBrand =
        machineFilters.brand === "All Brands" ||
        m.brand === machineFilters.brand;
      const matchesModel =
        machineFilters.model === "All Models" ||
        m.model === machineFilters.model;
      ...
      if (globalFilters) {
        if (
          globalFilters.machine.category &&
          m.category !== globalFilters.machine.category
        )
          matchesGlobal = false;
        if (globalFilters.machine.status) {
          ...
          if (sidebarStatus === "sold" && m.status !== "Sold")
            matchesGlobal = false;
          else if (sidebarStatus === "available" && m.status !== "Available")
            matchesGlobal = false;
          else if (sidebarStatus === "rented" && m.status !== "Active Rental")
            matchesGlobal = false;
        }
      }
      ...
      <div className="flex flex-col gap-6 min-h-0 h-full flex-1 w-full">
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-4 shrink-0">
          ...
          <StatCard
            ...
            accent="text-primary"
            trend={avgUtilization > 75 ? "Optimal" : ""}
            isUp={true}
          />
          <StatCard
            icon={Wrench}
          ...
          <div className="px-6 py-4 border-b border-border-main bg-bg-active/50 flex flex-col gap-4 shrink-0">
            <div className="flex items-center justify-between">
              <h3 className="text-[10px] font-black text-text-dim uppercase tracking-[0.2em] flex items-center gap-2">
                <FileText size={14} className="text-primary" /> MACHINE REPORTS
                DATA
              </h3>
            ...
            <div className="flex items-center gap-3 flex-wrap pb-2 pt-1">
              <select
                value={machineFilters.category}
                onChange={(e) =>
                  handleMachineFilterChange("category", e.target.value)
                }
              ...
              <select
                value={machineFilters.brand}
                onChange={(e) =>
                  handleMachineFilterChange("brand", e.target.value)
                }
                className="bg-bg-deep border border-border-main text-text-main text-[10px] rounded px-2 py-1 focus:outline-none focus:border-primary shrink-0"
              >
                {brands.map((b) => (
              ...
              <select
                value={machineFilters.model}
                onChange={(e) =>
                  handleMachineFilterChange("model", e.target.value)
                }
                className="bg-bg-deep border border-border-main text-text-main text-[10px] rounded px-2 py-1 focus:outline-none focus:border-primary shrink-0"
              >
                {models.map((m) => (
              ...
              <select
                ...
                onChange={(e) =>
                  handleMachineFilterChange("location", e.target.value)
                }
                className="bg-bg-deep border border-border-main text-text-main text-[10px] rounded px-2 py-1 focus:outline-none focus:border-primary shrink-0"
              >
                {locations.map((l) => (
              ...
              <select
                ...
                onChange={(e) =>
                  handleMachineFilterChange("status", e.target.value)
                }
                className="bg-bg-deep border border-border-main text-text-main text-[10px] rounded px-2 py-1 focus:outline-none focus:border-primary shrink-0"
              >
                {statuses.map((s) => (
              ...
              <select
                value={machineFilters.purchaseYear}
                onChange={(e) =>
                  handleMachineFilterChange("purchaseYear", e.target.value)
                }
                className="bg-bg-deep border border-border-main text-text-main text-[10px] rounded px-2 py-1 focus:outline-none focus:border-primary shrink-0"
              >
                {years.map((y) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-[9px] text-text-dim">
                        {row.brand} • {row.model} • {row.purchaseYear}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      ...
                      <div className="text-[9px] font-mono mt-1 text-text-dim">
                        {row.utilization}%
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
